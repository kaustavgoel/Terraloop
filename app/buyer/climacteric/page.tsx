...
import { ArrowLeft, Sun, Sparkles, ShoppingCart, Star, Clock, Phone, MapPin, User, CheckCircle2, X, Utensils, Droplets, Leaf } from "lucide-react"
...
import { useLocalization, type Language, getTranslation } from "@/lib/localization"

const climactericFruits = fruits.filter(f => f.category === "climacteric")

export default function ClimactericFruitsPage() {
  ...
  useEffect(() => {
    ...
    const listings = getListingsByCategory("climacteric")
    ...
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mb-12 text-center">
          <div className="mb-4 inline-flex items-center justify-center gap-3 rounded-full border border-amber-500/30 bg-amber-500/10 px-6 py-2">
            <Sun className="h-5 w-5 text-amber-400" />
            <span className="text-sm font-medium text-amber-300">{t.climactericDesc}</span>
          </div>
              ...
              {grocerListings.map((listing) => (
                <Card key={listing.id} className="card-magical group h-full overflow-hidden bg-card transition-all duration-300">
                  <CardContent className="p-0">
                    {/* Image container */}
                    <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-amber-900/20 to-orange-900/20">
                      {listing.images[0] && (
                        <Image
        ...
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {climactericFruits.map((fruit, index) => (
            <Link 
              key={fruit.id} 
              href={`/marketplace?category=climacteric&fruit=${fruit.id}`}
        ...
        <div className="mt-12 rounded-xl border border-amber-500/20 bg-amber-500/5 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="mb-1 font-semibold text-amber-300" style={{ fontFamily: 'Cinzel, serif' }}>
                Pro Tip: The Ripening Spell
              </h3>
              <p className="text-sm text-muted-foreground" style={{ fontFamily: 'Lora, serif' }}>
                Place climacteric fruits in a paper bag with a banana to speed up ripening. 
                The ethylene gas works like magic to bring out their sweetest flavors!
              </p>
            </div>
          </div>
        </div>
